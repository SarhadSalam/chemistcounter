 ...
 * <p>
 * The class ReactionManager is the algorithm used to balance reactions.
 */
public class ReactionManager
{
	 ...
	 */
	public static Double[] setMatrix(ReactionContainer u) throws ReactionNotBalancableException, ReactionElementNotMatchedException, ParserConfigurationException, SAXException, IOException, ElementNotFoundException, ParsingErrorException
	{
		ArrayList<String> productList = new ArrayList<>();
		ArrayList<String> compounds = new ArrayList<>();

//		The steps below are verification steps
		for( ElementDetails product : u.getProduct() )
		{
...
//		At this point, product list is equal to reactant list.
		...
		for( int i = 0; i<productList.size(); i++ )
		{
			...
			{
				ArrayList<ElementDetails> cn = new ArrayList<>();
				
				cn = ParseElements.parseElements(entireCompound.get(j).getName());
				
				for( ElementDetails chemicalName : cn )
				{
	...
	private static ReactionContainer setReactionBalance(ReactionContainer u, Double[] d)
	{
...
//		Calculates the minimum transpose multi-dim vector regardless of magnitude.
